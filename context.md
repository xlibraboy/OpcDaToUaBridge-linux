...
- `BridgeState.SetSourceError` marks the source `"Faulted"` and surfaces the message; aggregate `DaConnectionState` becomes `"Partial"` if some sources are connected. Paused sources (see `/api/da/sources/pause` above) are intentionally offline, not failures: `AggregateConnectionState` treats `"Paused"` as neutral (all-paused → aggregate `"Paused"`; paused + faulted → `"Faulted"`; paused + connected → `"Connected"`), and `BridgeState.ClearSourceError` wipes a stale fault text without touching the state. A **connect** failure follows the same split as a read failure: transient causes surface as `SourceConnectionLostException` and are retried with backoff as `"Reconnecting"`, terminal config errors stay `Faulted`. MX Component is classified by `MxConnectErrorClassifier` (`OpcBridge.Drivers.MxComponent`): an `Open()` failure is transient — the logical station may still be claimed by the session a pause just released (#5) — while MX Component not being registered (not installed) and non-Windows are terminal.
...
- **Client credentials (#14):** with `RequireAuthentication` on, `BridgeUaServer.SelectUserTokenPolicies` advertises the username/password policy alone, as a plaintext (token policy `None`) token — the channel is None-only, and a Basic256Sha256-encrypted token is not decryptable on that path (every login fails with a null password), so the token policy has to match. `ValidateUserIdentity` runs in `ActivateSessionAsync`, the overload the stack actually calls; the obsolete `SessionServerBase.ActivateSession(...)` override is a pass-through to the same check (validating *only* there left the gate unenforced — a wrong password still activated a session). Anonymous identities get `BadUserAccessDenied`, a wrong credential `BadIdentityTokenInvalid`. `UaServerHost.UpdateOptions` persists to `ua-settings.json`, so the gate survives restarts and applies on the next one. Operator surface: **Monitor ▸ UA Server Access** (`DashboardPage.loadUaAccess`/`saveUaAccess`).
...
- `GET /api/da/sources` — source registry (each row carries `paused`); `POST /api/da/sources` (upsert); `POST /api/da/sources/remove`; `POST /api/da/sources/update-rate`; `POST /api/da/update-rate`; `POST /api/da/sources/pause` — `{"sourceId":..., "paused":true|false}` temporary runtime pause (#5, Engineer role): the worker disposes the source's session and reports `Paused` until resumed; the flag is **never** persisted to sources.json, so a restart resumes everything, and config import clears it (dashboard Pause/Resume control: the MX Component list only — that COM port is the case it exists for). Pausing releases the upstream session for real: `ActUtlTypeSession.CloseCore` calls `Close()` and then `Marshal.FinalReleaseComObject`, because dropping the COM reference alone left the logical station claimed until a GC and the next `Open()` failed against the session that had just been paused. Resume reconnects on the next reconcile pass, and a transient re-open failure is retried with backoff as `"Reconnecting"` — never parked in `Faulted`, which is what made a failed resume permanent (#5).
...
- `GET /api/ua/settings` | `POST /api/ua/settings` — built-in UA server config (Engineer role): GET reports `requireAuthentication` + `username` and never the stored password; POST validates and persists to `ua-settings.json`, applying on the next bridge restart. Enabling the requirement without a usable username/password (in the request or already stored) is refused with `400` — it would lock every client out; a blank **field** means "no change", so the dashboard's cleared password box never wipes the secret (#14).
...
- **Never read a mirror node's `WrappedValue` on the value path.** A mapping whose DataType is `Auto` gets a node whose declared DataType is the abstract `BaseDataType` (`ToDataTypeId`), and `BaseVariableState.WrappedValue` throws `InvalidCastException` for a raw value on such a node (`Unable to cast object of type 'System.Boolean' to type 'Opc.Ua.Variant'`). The update runs inside the poll cycle, so that exception faulted every Auto-typed source and cleared its readings on the plant host (fixed in `fix/ua-bandwidth-base-data-type`): build a `Variant` from the value instead (`BridgeNodeManager.WrapForMeasurement`, the shape the stack itself builds for a read and a notification). A diagnostic must never be able to fault a source.
