...
public sealed class DaRecoveryCoordinatorTests
{
    ...
    private sealed class RecoveringSourceClient : ISourceClient, ISubscribableSourceClient, ISubscriptionActiveSource
    {
        ...
        {
            int call = Interlocked.Increment(ref connectCalls_);
            if (call <= failingConnectAttempts_)
            {
                throw new SourceConnectionLostException("test: OPC DA server unreachable");
            }
    ...
    {
        private readonly SubscriptionSourceClient client_;

        public SubscriptionSourceClientFactory(SubscriptionSourceClient client)
        {
            client_ = client;
        }
