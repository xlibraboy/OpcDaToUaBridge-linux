...
- Tags set to "Source Default" (update rate = 0) inherit the global **Default Update Rate** (Connectivity → OPC DA).
...
- Subscriptions can be toggled in **Connectivity → OPC DA → DA Subscriptions**. When ON (default), the bridge uses `IOPCDataCallback` to receive value changes from the DA server instead of polling with `IOPCSyncIO.Read`. Changing the toggle takes effect on the next source reconnect.
...
```
 Connectivity → OPC DA → Credentials section
...
- **Manual ProgID** — if a server doesn't appear in scan, type the ProgID directly in Connectivity → OPC DA → ProgID field and provide credentials
...
- **DA browse fails** — check ProgID, host reachability, DCOM permissions, and credentials (Connectivity → OPC DA → Credentials section).
...
- **Deadband not filtering** — deadband only works under **subscriptions** (Connectivity → OPC DA → DA Subscriptions ON). If the DA server doesn't support `IOPCDataCallback`, the bridge falls back to polling and deadband has no effect. Check Logs for the subscription fallback warning.
...
Use the **Connectivity → OPC DA → Backup & Restore** section in the dashboard:
...
2. **Export a backup** — Connectivity → OPC DA → Backup & Restore → **Export Config** (saves all sources + mappings to a JSON file). This is your safety net if anything goes wrong.
...
| Sources | Connectivity → Sources / OPC DA | All your sources are still there |
...
3. **Lost mappings** — restore from the backup JSON: Connectivity → OPC DA → Backup & Restore → Import Config
...
- **Da:UpdateRateMs** — default update rate for new sources (min 100ms); can be changed live in Connectivity → OPC DA → Default Update Rate
- **Da:UseSubscriptions** — use `IOPCDataCallback` subscriptions (default `true`); can be toggled live in Connectivity → OPC DA → DA Subscriptions
