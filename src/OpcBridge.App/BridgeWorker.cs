...
public sealed class BridgeWorker : BackgroundService, IDaLinkMetadataResolver
{
    ...
    private readonly ConcurrentDictionary<string, int> consecutive_failures_ = new(StringComparer.OrdinalIgnoreCase);
                    ...
                    try
                    {
                        ...
                        if (!failedSourceQueue.IsEmpty)
                        {
                            ...
                            foreach (string failedId in failedIds)
                            {
                                ...
                                consecutive_failures_.TryRemove(failedId, out _);
        ...
        CancellationToken pollerToken)
    {
        while (!pollerToken.IsCancellationRequested)
        {
            ...
            try
            {
                ...
                {
                    if (!ShouldTearDownAfterFailure(source))
                    {
                        logger_.LogWarning(
                            "Source {SourceId} rate {Rate}ms read failed ({Count}/{Max} consecutive); keeping session",
                            source.SourceId,
                            rate,
                            consecutive_failures_.GetValueOrDefault(source.SourceId),
                ...
                {
                    consecutive_failures_.TryRemove(source.SourceId, out _);
                }
            }
    ...
    private bool ShouldTearDownAfterFailure(DaSourceRuntimeSettings source)
    {
        int max = source.MaxConsecutiveFailures < 1 ? 1 : source.MaxConsecutiveFailures;
        int count = consecutive_failures_.AddOrUpdate(source.SourceId, 1, (_, current) => current + 1);
        return count >= max;
    }

    /// <summary>
    /// Subscription-mode health check: when a DA subscription is active the poller performs
    /// no device reads, so a dead server is only detectable through <c>IOPCDataCallback</c>
    /// traffic. A source whose callbacks have stopped arriving for longer than
    /// <see cref="DaSourceRuntimeSettings.WatchdogTimeoutMs"/> is enqueued for teardown and
    /// reconnect. Sources that never delivered a callback (e.g. static tags) are left alone
    /// so quiet-but-healthy subscriptions do not flap.
    /// </summary>
    private void ScanWatchdog(Dictionary<string, SourceSession> sessions, ConcurrentQueue<string> failedSourceQueue)
    {
        ...
        foreach (SourceSession session in sessions.Values)
        {
            ...
            logger_.LogWarning(
                "Source {SourceId} subscription watchdog: no callbacks for {ElapsedMs}ms (limit {TimeoutMs}ms); reconnecting",
        ...
        for (int i = 0; i < settings.Sources.Count; i++)
        {
                ...
                && SourceConnectionEquals(existing.Source, source))
            {
                ...
                if (!SourceSettingsEquals(existing.Source, source))
                {
                    ...
                    // Rate/subscription/recovery-knob changes still need poller restart
                    // so pollers pick up the new source settings.
                    if (existing.Source.UpdateRateMs != source.UpdateRateMs
                        || existing.Source.UseSubscriptions != source.UseSubscriptions
                        || existing.Source.MaxConsecutiveFailures != source.MaxConsecutiveFailures
                        || existing.Source.WatchdogTimeoutMs != source.WatchdogTimeoutMs)
                    {
